...
// Generate AI response using Gemini via OpenAI-compatible endpoint
async function generateResponse(contact, incomingMessage, messageHistory) {
  const historyText = messageHistory.slice(0, 5)
    .map(m => `${m.is_from_me ? 'Me' : contact.name || contact.identifier}: ${m.text || '[attachment]'}`)
    .join('\n');
...
Generate a response now:`;
  ...
  // Optional fallback: try to load from Clawdbot config if env var missing
  if (!apiKey) {
    const configPath = path.join(os.homedir(), '.clawdbot', 'clawdbot.json');
    if (fs.existsSync(configPath)) {
      try {
        const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        apiKey = config.apiKeys?.gemini || config.skills?.gemini?.apiKey;
      } catch (e) {
        // Ignore parse errors
      }
    }
  ...
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY not found in environment or Clawdbot config');
  }

  // Create OpenAI client pointed at Gemini's compatible endpoint
  ...
  try {
    const completion = await openai.chat.completions.create({
      model: 'gemini-3-flash-preview', // or 'gemini-1.5-pro-latest' if you have access
      messages: [{ role: 'user', content: prompt }],
      max_tokens: 300,
      temperature: 0.9,
    });
    ...
    const disclosure = `\n\n(This reply was generated by Gemini AI and auto-sent after a short delay.)`;
...
// Send message via imsg using the correct service (iMessage or SMS)
async function sendMessage(identifier, text) {
  return new Promise(async (resolve, reject) => {
    try {
      // Step 1: Get chat list to determine service type
      const chats = await getChatList();
      const chat = chats.find(c => c.identifier === identifier);
      
      if (!chat) {
        reject(new Error(`No chat found for identifier: ${identifier}`));
        return;
      }

      // Map service names: "iMessage" -> "imessage", "SMS" -> "sms"
      const service = chat.service.toLowerCase(); // "imessage" or "sms"
      log(`Sending via ${service} to ${identifier}`);

      // Step 2: Send using the correct service
      const args = ['send', '--to', identifier, '--text', text, '--service', service];

      const proc = spawn('imsg', args);
      let errorOutput = '';
      ...
      proc.on('close', (code) => {
        ...
        log(`✓ Message sent successfully via ${service}`);
        resolve();
      });
    } catch (error) {
      reject(error);
